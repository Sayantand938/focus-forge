...
# ID_MAPPING_FILE = DB_DIR / "id_mapping.json" # Path for the json file
...
#     if cursor.rowcount > 0:  # More robust check for successful deletion
...
#     if end_time_str is None:  # Only check start time
#         query = """
...
#         (start_time >= ? AND end_time <= ?)    -- New session is entirely within existing
#     )
#     """
...
#     # 2.  Prepare updated values, defaulting to original if not provided
...
#     # 4. Construct datetime objects for calculations
#     try:
#         # Correctly handle different update scenarios using HH:MM:SS
#         if updated_start_time and updated_end_time:  # Both times changed
...
#         elif updated_start_time:  # Only start time changed
#             start_datetime = datetime.strptime(f"{updated_date} {updated_start_time}", "%Y-%m-%d %H:%M:%S")
#             end_datetime = datetime.strptime(f"{updated_date} {original_end_time}", "%Y-%m-%d %H:%M:%S")  # Use updated date with original end time
#         elif updated_end_time:  # Only end time changed
#             start_datetime = datetime.strptime(f"{updated_date} {original_start_time}", "%Y-%m-%d %H:%M:%S") # Use updated date with original start time
#             end_datetime = datetime.strptime(f"{updated_date} {updated_end_time}", "%Y-%m-%d %H:%M:%S")
#         elif new_date: # Only Date is Changed
...
#     # 6. Check for overlaps (excluding the current session)
...
#     # 7. Update the database (using HH:MM:SS)
#     cursor.execute("""
#         UPDATE focus_forge
...
#     try:
#         # Load existing mapping or create an empty one
...
#         # Fetch all current data (use the same filtering as 'list')
...
#             db_id = row[0]  # The actual database ID
#             new_mapping[str(serial_num)] = db_id #mapping serial no with db id
...
#         logger.error(f"Error updating ID mapping: {e}")
#         raise
...
import re
import json
from .config import DB_FILE, ID_MAPPING_FILE, DB_DIR  # Import configs

logger = logging.getLogger(__name__)

# Define the database file path
# HOME_DIR = Path.home() # Removed
# DB_DIR = HOME_DIR / ".focus-forge" # Removed
# DB_FILE = DB_DIR / "focus-forge.db" # Removed
# ID_MAPPING_FILE = DB_DIR / "id_mapping.json" # Path for the json file # Removed

# Ensure the database directory exists
# DB_DIR.mkdir(parents=True, exist_ok=True) # Moved to config
